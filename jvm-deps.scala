...
// mechanism — no `--exclude` flags are needed for dependency scoping; the one remaining
// `--exclude test/js/integration` on plain-JS test runs is a linker workaround, see AGENTS.md.)
//
