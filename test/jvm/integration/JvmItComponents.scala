...
object JvmItComponents:
  ...
  /** A rendered resource tree: the temp root, its rendered component file Paths, the keys dir and the secrets.json file
    * (ready to mount into daprd).
    */
  final case class Rendered(root: Path, components: List[Path], keysDir: Path, secretsFile: Path)
