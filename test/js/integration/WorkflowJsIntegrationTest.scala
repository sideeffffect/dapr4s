...
/** [[WorkflowCapability]] against the workflow runtime hosted in-process by [[ServerDaprJsItSuite]]
  * ([[dapr4s.internal.WorkflowHost]] + the AsyncGenerator coroutine bridge), backed by the testcontainer sidecar's
  * scheduler service — the Scala.js twin of [[WorkflowCapabilityServerTest]].
  *
...
class WorkflowJsIntegrationTest extends FunSuite, ServerDaprJsItSuite:
