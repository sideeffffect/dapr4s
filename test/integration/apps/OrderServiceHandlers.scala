...
/** Business logic for the Order microservice.
  ...
  *
  * '''`try { ... } catch case e: Exception => throw e` in each handler lambda''' WHY: In Scala 3.9 CC, each lambda that
  * calls a `throws`-annotated method creates a fresh anonymous `CanThrow` capability. Sibling lambdas (defined in the
  * same method body) cannot share these capabilities — a CC constraint that prevents CanThrow "leaking" across
  * unrelated lambdas. The try/catch absorbs the CanThrow at each lambda's boundary so the next sibling lambda starts
  * fresh. Without this, the second and later lambdas fail to compile with "capability `any` cannot flow into capture
  * set {any²}". The re-throw preserves the original exception without swallowing it. See the AGENTS.md section on CC
  * sibling lambda patterns.
  */
