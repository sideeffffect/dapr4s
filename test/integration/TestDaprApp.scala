...
/** Test helper for exercising [[DaprApp]] handler logic in-process.
  *
  * Instead of starting an HTTP server, the `call` and `deliver` methods
  * dispatch directly to the registered handlers captured in a [[DaprApp]] —
  * allowing integration tests to invoke business logic against a real Dapr
  * sidecar without any HTTP round-trips.
  *
  * {{{
  ...
  * }}}
  *
  * WHY @assumeSafe: the handler lambdas stored in [[DaprApp]] capture DAPR
  * capabilities (state, pub/sub, etc.) from the scope that created the app.
  * Calling those lambdas and threading through the codec encode/decode round-trip
  * that bridges the existential [[InvocationRoute]] / [[Subscription]] type members
  * to the caller's concrete types both require bypassing CC tracking — the standard
  * library-boundary pattern.
  */
@scala.caps.assumeSafe
object TestDaprApp:

  /** Invoke a registered method handler directly, encoding `request` with the
    * caller's codec and decoding the response back to `Resp`.
    *
    * The encode → decode round-trip bridges the caller's concrete `Req`/`Resp`
    * types to the existential `inv.Req`/`inv.Resp` types without `asInstanceOf`.
    * Fails fast if no handler is registered for `method`.
    */
  def call[Req: JsonCodec](app: DaprApp, method: String, request: Req)[Resp: JsonCodec]: Resp =
    val inv = app.invocations.find(_.methodName.value == method).getOrElse(
  ...
  /** Deliver a pub/sub event directly to a registered subscriber.
    *
    * The encode → decode round-trip bridges the caller's concrete `T` to the
    * existential `sub.Payload` type; `sub.codec` and `sub.rawHandler` are then
    * used in a type-coherent way via path-dependent types on `sub`.
  ...
  def deliver[T: JsonCodec](app: DaprApp, topic: String, event: CloudEvent[T]): SubscriptionResult =
    val sub     = app.subscriptions.find(_.topic.value == topic).getOrElse(
    ...
    val data: sub.Payload = sub.codec.decode(dataJson) match
      case Right(v) => v
      case Left(e)  => throw e
    handler(CloudEvent[sub.Payload](
      id              = event.id,
      source          = event.source,
