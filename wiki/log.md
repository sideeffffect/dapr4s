...
## [2026-06-17] note | server-delivery IT suites unified into test/shared (actor/pub-sub/invoke/workflow)
- EXTENDS the 2026-06-13 unification: the server-delivery suites — previously the last platform-specific holdouts (JVM ActorCapabilityServerTest/WorkflowCapabilityServerTest/PublishCapabilityServerTest/PubSubIntegrationTest, JS *JsIntegrationTest) — are now SINGLE cross-platform classes in test/shared/integration: ActorItTest, PubSubItTest, WorkflowItTest, plus InvokeItTest (already shared) all mix a new `ServerDaprItSuite` fixture. Test bodies call only the *client* capabilities (ActorCapability.invoke / publish+state / WorkflowCapability) + ItPolling (eventually/retryUntilSuccess), so they're byte-identical across platforms. The JVM bodies that previously poked raw `HttpURLConnection` against the sidecar's actor API were rewritten to ActorCapability.invoke — that raw HTTP was just the capability call spelled out by hand.
- `ServerDaprItSuite` is defined once per platform under the same name (cross-build idiom, like SharedDaprItSuite). Both impls host the SAME shared union app `itUnionApp` = ItServerApp ++ CounterActorApp ++ WorkflowApp ++ GatedWorkflow (moved JsTestServer's JsItServerApp/jsItUnionApp/GatedWorkflow into test/shared/apps/ItServerApp.scala; neutral `it-*` topic names). JVM hosts it via `Dapr(config).serve(itUnionApp)` on a virtual thread — the exact analogue of JS `Dapr(cfg).serveAsync` — which auto-wires the actor sidecarHttpEndpoint and the workflow gRPC `workflowProperties` from config.sidecar. Key ordering fact: start sidecar FIRST (workflow runtime needs the gRPC endpoint at server-start), so the JVM fixture overrides DaprTestContainer's default `/v1.0/healthz` wait (needs the app channel → would deadlock) with `/v1.0/healthz/outbound`, and uses `withAppHealthCheckPath("/dapr/config")` so daprd waits for the app then establishes the channel (registering subs + actor types). Mirrors what JS already did.
- Coverage went UP, not just consolidated: each platform inherits the other's previously-unique cases (JVM gains the gated-workflow raiseEvent path; JS gains workflow startWithId/getStatus/purge and the actor reminder/timer callback loops). The genuinely platform-specific residue — JVM DaprAppServer HTTP routing/status-code checks (404, /dapr/config, DELETE) + the DaprApp ++ merge — moved to the JVM unit ActorServerRoutingTest (no sidecar). Jobs/Conversation*ServerTest stay JVM-only (no JS SDK). Verified both platforms compile (scala-cli 1.14 via coursier; local 1.12.2 is too old) + scalafmt clean; real-sidecar behavior gated by CI (needs Docker + Node 25).

## [2026-06-13] note | full JVM/JS test unification — one component set, redis everywhere, shared scenarios
- SUPERSEDES the 2026-06-12 "component-config mechanism stays platform-idiomatic" note below. The two platforms now SHARE the component definitions: scripts/it/components/*.yaml + scripts/it/secrets.json are the single source of truth, rendered per topology by scripts/it/render-components.sh (only env-specific value is redisHost — localhost:6391 for the JS host-network harness, redis:6379 for the JVM testcontainers network). The JS harness mounts the rendered dir into daprd; the JVM feeds the SAME files via `io.dapr.testcontainers.DaprContainer.withComponent(java.nio.file.Path)` (the file-ingesting overload — present in testcontainers-dapr 1.17.2, this is what makes shared files possible). No scripts/jvm-it/ twin needed and none of the old in-memory divergence: both platforms run state/pubsub/lock/configuration on redis, secrets on local.file, crypto on localstorage.
- Test logic is shared via traits in test/shared/scenarios (self: munit.Assertions =>, shared API + given DaprCapability). JVM and JS suites are thin shells owning only bring-up + the sync/Future boundary, then calling the same scenario methods. Direct-call capabilities (state/secrets/lock/crypto/configuration/invoke) fully share call+assertions; the JVM gained SharedDaprItSuite (single all-components redis sidecar) + JvmItComponents (renderer). Server-delivery suites (actor/workflow/pubsub delivery, app-level Order/Inventory/EndToEnd) keep platform-specific bring-up (a host DaprAppServer the sidecar calls back into vs the external JsTestServer) but run on the shared redis components via RedisFixture. Closed the JS invoke error-path gap (non-existent-app throws — was JVM-only). Verified: every JVM integration suite + the full JS Wasm+JSPI leg green on redis. Design in docs/JVM-JS-PARITY.md.
- Redis etag gotcha encoded in the shared StateScenarios: a stale-but-real (numeric) etag is required to provoke a conflict — a fabricated non-numeric string gets a 400 from daprd, not a 409 (the JVM suites previously hid this by using state.in-memory, which tolerates any mismatched etag).
