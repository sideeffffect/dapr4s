...
import io.dapr.actors.client.{ActorClient as JavaActorClient, ActorProxy as JavaActorProxy, ActorProxyBuilder as JavaActorProxyBuilder}
...
/** Client-side capability for invoking methods on a specific Dapr virtual actor instance.
  ...
  * by the same codec — bypassing the Java SDK's Jackson-based serializer entirely.
  */
...
private[safe] final class ActorCapabilityImpl(
  ...
  def invokeVoid(method: MethodName): Unit throws DaprActorException =
    ...
    catch case e: Exception => throw DaprActorException(s"Actor invocation '${actorType.value}/${actorId.value}/${method.value}' failed: ${e.getMessage}", e)
  ...
  private def decodeResponse[Resp: JsonCodec](method: MethodName, rawResult: Array[Byte] | Null): Resp =
    ...
    summon[JsonCodec[Resp]].decode(responseStr) match
      case Left(err) => throw JsonDecodeException(s"Actor '${actorType.value}/${method.value}' response decode failed: ${err.getMessage}")
      case Right(v)  => v
