...
/** Internal bridge: wraps a [[DaprWorkflow]] instance for registration with the Dapr Java SDK.
  *
...
private[safe] final class DaprWorkflowBridge(
    private val workflow: DaprWorkflow,
...
/** Internal bridge: wraps a [[DaprActivity]] instance for registration with the Dapr Java SDK.
  *
  * Uses instance-based `registerActivity(name, instance)` so that the activity is registered under the user's
  * canonical class name, matching the name generated by [[WorkflowContextImpl.callActivity]].
  */
@scala.caps.assumeSafe
private[safe] final class DaprActivityBridge[I, O](
    private val activity: DaprActivity[I, O],
