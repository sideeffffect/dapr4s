...
/** Filesystem path to a PEM file (TLS certificate, private key, or CA bundle).
  ...
  * configuration cross-compiles to Scala.js, where `java.nio.file` does not exist. On the JVM, convert with
  * `PemPath(path.toString)` or `java.nio.file.Path.of(pemPath.value)`.
  *
  * Must not be empty.
  */
